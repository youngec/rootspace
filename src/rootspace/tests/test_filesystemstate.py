...
def test_read_file(file_system_state):
    data = file_system_state.read(0, 0, "/etc/passwd")

    assert data == test_fs_database[0x0001]
    assert id(data) != id(test_fs_database[0x0001])


def test_read_directory(file_system_state):
    with pytest.raises(IsADirectoryError):
        file_system_state.read(0, 0, "/etc")


def test_write_file(file_system_state):
    file_system_state.write(0, 0, "/etc/shadow", "Root is 1337!")


def test_write_directory(file_system_state):
    with pytest.raises(IsADirectoryError):
        file_system_state.write(0, 0, "/etc", None)


def test_execute_executable(file_system_state):
    assert file_system_state.execute(0, 0, "/bin/test", None) == 0


def test_execute_non_executable(file_system_state):
